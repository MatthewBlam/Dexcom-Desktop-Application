...
export const DexcomG6 = forwardRef<HTMLDivElement, DexcomProps>(({ trend, mg_dl, mmol_l, children, className, ...props }, ref) => {
  ...
  return (
    <div ref={ref} className={twMerge("flex w-full h-full", className)} {...props}>
      <div id="dexcom" className="relative scale-dscale w-full h-full -mt-[1%]">
        <div id="dexcom_body" className={twMerge(variant.rotate, "absolute origin-center left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2")}>
          <svg xmlns="http://www.w3.org/2000/svg" width="250" height="250" viewBox="0 0 250 250">
            <path
              ...
              style={{
                fill: "var(--color-dex-g6-arrow)",
                opacity: variant.hidden ? "0" : "1",
              }}
            />
            <svg id="circle-NODATA" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 250 250">
              <circle
                cx="125"
                cy="125"
                r="125"
...
export const DexcomG7 = forwardRef<HTMLDivElement, DexcomProps>(({ trend, mg_dl, mmol_l, children, className, ...props }, ref) => {
  ...
  const textColor = range === "low" ? "text-white" : "text-dex-text-dark";
  const unitTextColor = range === "low" ? "text-white" : range === "high" ? "text-dex-text-dark" : "text-dex-text-muted";

  const variant = trendVariantsG7(trend);
  return (
    <div ref={ref} className={twMerge("flex w-full h-full bg-transparent", className)} {...props}>
      <div id="dexcom" className="relative scale-dscale w-full h-full -mt-[1%]">
        <div id="dexcom_body" className={twMerge(variant.rotate, "drop-shadow-3xl origin-center absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2")}>
          <svg xmlns="http://www.w3.org/2000/svg" width="250" height="250" viewBox="0 0 250 250">
            <defs>
              <filter id="shadow">
            ...
            <circle
              id="dexcom_circle"
              className={twMerge(variant.reverse, "rounded-full origin-center")}
              cx="125"
              cy="125"
              r="110.6714"
              style={{
                fill: circleColor,
                filter: range !== "normal" ? "" : "url(#shadow)",
              }}
            />
            <path
              className="arrow"
