...
export const Display = forwardRef<HTMLDivElement, DisplayProps>(
  ({ reading, connectionStatus, openSettings, toggleWidget, widgetOpen, tabbable, historyExpanded, onToggleHistoryExpanded, children, className, ...props }, ref) => {
      ...
      <div ref={ref} className={twMerge("flex flex-col w-full h-full px-8 py-5", className)} {...props}>
        <div className="flex flex-col w-full flex-1 min-h-0">
          ...
          <div className="relative w-full flex-1 min-h-0 mb-1">
            {sensorSetting === "G7" ? <DexcomG7 trend={trend} mg_dl={String(mg_dl)} mmol_l={String(mmol_l)}></DexcomG7> : <DexcomG6 trend={trend} mg_dl={String(mg_dl)} mmol_l={String(mmol_l)}></DexcomG6>}
          </div>
          <History open={true} expanded={historyExpanded} onToggleExpanded={onToggleHistoryExpanded} connectionStatus={connectionStatus} className=""></History>
        </div>
      </div>
    );
  },
